...
describe("Contract2 Tests", function () {
	it("second contract - simple", async function () {
		...
		const missingArgumentArrayErr = {
			name: "Error",
			message:
				"missing argument: passed to contract (count=0, expectedCount=1, code=MISSING_ARGUMENT, version=contracts/5.6.2)",
		};
		let err0;
		try {
		...
		const nullPointerArrayError = {
			name: "Error",
			message:
				'call revert exception [ See: https://links.ethers.org/v5-errors-CALL_EXCEPTION ] (method="arr(uint256)", data="0x", errorArgs=null, errorName=null, errorSignature=null, reason=null, code=CALL_EXCEPTION, version=abi/5.6.4)',
		};
		let err;
		try {
...
describe("contract 6", function () {
	...
	it("calling non payable function throws error", async () => {
		...
		expect(e.message).equal(
			'non-payable method cannot override value (operation="overrides.value", value={"type":"BigNumber","hex":"0x022b1c8c1227a00000"}, code=UNSUPPORTED_OPERATION, version=contracts/5.6.2)'
		);
	});
});
